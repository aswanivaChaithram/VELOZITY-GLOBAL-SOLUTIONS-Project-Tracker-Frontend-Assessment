...
const Timeline = ({ tasks }: Props) => {
  ...
  const totalDays = Math.ceil(
      (maxDate.getTime() - minDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  ...
  return (
    <div className="h-full overflow-auto bg-[#1E1F22] p-4">
      <div className="min-w-[900px] relative">
        ...
        <div className="absolute top-[60px] bottom-0 w-[2px] bg-blue-400 z-10"
        ...
        <div className="flex flex-col gap-4 relative">
          ...
          {tasks.map((task) => {
            ...
            const left = hasStart ? 
            getPosition(task.startDate!) : getPosition(task.dueDate);

            const width = hasStart ? 
            getWidth(task.startDate!, task.dueDate) : 1.5;
            ...
            return (
              <div key={task.id} className="relative h-10 flex items-center">
                ...
                <div className="flex-1 relative h-6 bg-[#2A2B2E] rounded">
                  ...
                  <div className={`absolute h-6 rounded ${getPriorityColor(task.priority)}`} 
